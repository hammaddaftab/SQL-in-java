...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        app.get("/admin", ctx -> {
            if (!requireAdmin(ctx)) return;
            ctx.json(Map.of(
                "ordersToday",     dao.countOrdersToday(),
                "pendingOrders",   dao.countOrdersByStatus("pending"),
                "lowStockItems",   dao.countLowStockItems(),
                "pendingRestocks", dao.countPendingRestocks(),
                "salesToday",      dao.getTotalSalesToday(),
                "topProducts",     dao.getTopSellingProducts(5)
            ));
        });
        ...
        app.post("/admin/products", ctx -> {
            if (!requireAdmin(ctx)) return;
            String name = ctx.formParam("name"); String priceStr = ctx.formParam("price"); String category = ctx.formParam("category");
            if (name == null || priceStr == null || category == null) { ctx.status(400).json(Map.of("error", "name, price, category required")); return; }
            Product p = new Product(); p.name = name; p.price = Double.parseDouble(priceStr); p.category = category;
            ctx.status(201).json(Map.of("productID", dao.addProduct(p)));
        });

        app.post("/admin/products/{id}/update", ctx -> {
            if (!requireAdmin(ctx)) return;
            int id = Integer.parseInt(ctx.pathParam("id")); Product p = dao.getProductById(id);
            if (p == null) { ctx.status(404).json(Map.of("error", "Product not found")); return; }
            String name = ctx.formParam("name"); String priceStr = ctx.formParam("price"); String category = ctx.formParam("category");
            if (name != null) p.name = name; if (priceStr != null) p.price = Double.parseDouble(priceStr); if (category != null) p.category = category;
            dao.updateProduct(p); ctx.json(Map.of("status", "ok"));
        });

        app.post("/admin/products/{id}/delete", ctx -> {
            if (!requireAdmin(ctx)) return;
            dao.deleteProduct(Integer.parseInt(ctx.pathParam("id"))); ctx.json(Map.of("status", "ok"));
        });

        app.get("/admin/orders", ctx -> {
            if (!requireAdmin(ctx)) return;
            String statusFilter = ctx.queryParam("status");
            List<CustomerOrder> orders = (statusFilter != null && !statusFilter.isEmpty()) ? dao.getOrdersByStatus(statusFilter) : dao.getAllOrders();
            List<Map<String, Object>> orderData = new ArrayList<>();
            for (CustomerOrder o : orders) {
                Customer c = dao.getCustomerById(o.customerID);
                String customerName = (c != null) ? c.firstName + " " + c.lastName : "Guest";
                Map<String, Object> m = new HashMap<>();
                m.put("orderID", o.orderID); m.put("customerID", o.customerID); m.put("customerName", customerName);
                m.put("tableID", o.tableID); m.put("time", o.time); m.put("paymentMethod", o.paymentMethod);
                m.put("isOnline", dao.isOnlineOrder(o.orderID)); orderData.add(m);
            }
            ctx.json(Map.of("orders", orderData));
        });
        ...
        app.post("/admin/orders/{id}/status", ctx -> {
            ...
            if (newStatus == null) { ctx.status(400).json(Map.of("error", "status required")); return; }
            dao.updateOrderStatus(Integer.parseInt(ctx.pathParam("id")), newStatus); ctx.json(Map.of("status", "ok"));
        });

        app.post("/admin/orders/{id}/confirm", ctx -> {
            if (!requireAdmin(ctx)) return;
            int orderId = Integer.parseInt(ctx.pathParam("id"));
            dao.confirmOnlineOrder(orderId); dao.updateOrderStatus(orderId, "confirmed"); ctx.json(Map.of("status", "ok"));
        });

        app.get("/admin/inventory", ctx -> {
            if (!requireAdmin(ctx)) return;
            ctx.json(Map.of("ingredients", dao.getAllIngredients(), "suppliers", dao.getAllSuppliers()));
        });

        app.post("/admin/inventory/restock", ctx -> {
            if (!requireAdmin(ctx)) return;
            String ingIdStr = ctx.formParam("ingredientID"); String supIdStr = ctx.formParam("supplierID"); String qtyStr = ctx.formParam("quantity");
            if (ingIdStr == null || supIdStr == null || qtyStr == null) { ctx.status(400).json(Map.of("error", "ingredientID, supplierID, quantity required")); return; }
            RestockRequest r = new RestockRequest(); r.ingredientID = Integer.parseInt(ingIdStr); r.supplierID = Integer.parseInt(supIdStr);
            r.quantityRequested = Integer.parseInt(qtyStr); r.status = "pending"; r.requestedAt = System.currentTimeMillis() / 1000;
            ctx.status(201).json(Map.of("requestID", dao.addRestockRequest(r)));
        });
        ...
        app.get("/admin/sales", ctx -> {
            ...
            ctx.json(Map.of(
                ...
                "topProducts",     dao.getTopSellingProducts(10)
            ));
        });
